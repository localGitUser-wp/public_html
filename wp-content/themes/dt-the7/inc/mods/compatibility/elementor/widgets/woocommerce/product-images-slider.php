...
class Product_Images_Slider extends The7_Elementor_Widget_Base {
	...
	protected function render() {
		...
		<div class="thumbs-swiper">
			...
			<?php $this->render_thumbs_arrows( $settings ); ?>
		</div>
		<?php
	}
	...
	protected function render_thumbs_arrows( $settings ) {
		?>
		...
		$this->add_control(
			...
			[
				...
				'selectors_dictionary' => [
					'vertical'   => ' top: calc(100% + var(--bullets-v-offset, 10px)); --bullets-direction: column nowrap;',
					'horizontal' => 'left: calc(50% + var(--bullets-h-offset, 0px)); --bullets-direction: row wrap;',
				],
				'selectors'            => [
		...
		$this->update_control(
			...
			[
				'selectors_dictionary' => [
					...
					'bottom' => 'top: auto; bottom: var(--bullet-v-offset, 10px); --bullet-translate-y:0;',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'device_args'          => [
					'tablet' => [
						'default' => '',
						'options' => $pagination_options_on_devices,
					],
					'mobile' => [
						'default' => '',
						'options' => $pagination_options_on_devices,
	...
	protected function add_thumbs_style_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default'              => 'vertical',
				'selectors_dictionary' => [
					'vertical'   => ' top: calc(100% + var(--thumbs-v-offset, 0px)); max-width: 100%; max-height: var(--max-width, 100%); --thumb-width: auto; --thumb-height: var(--thumb-item-size, 80px)',
					'horizontal' => 'left: calc(50% + var(--thumbs-h-offset, 0px)); max-width: var(--max-width, 100%); max-height: 100%; --thumb-height: auto; --thumb-width: var(--thumb-item-size, 80px)',
				],
				'selectors'            => [
				...
				'frontend_available'   => true,
				'render_type'          => 'template',
				'prefix_class'         => 'pagination-position-',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors_dictionary' => [
					'top'    => 'top: var(--thumbs-v-offset, 0px); bottom: auto; --thumbs-translate-y:0; --justify-v-thumb-content: flex-start; --swiper-t-margin: 0; --swiper-b-margin: auto;',
					'center' => 'top: calc(50% + var(--thumbs-v-offset, 0px)); bottom: auto; --thumbs-translate-y:-50%; --justify-v-thumb-content: center; --swiper-t-margin: auto; --swiper-b-margin: auto;',
					'bottom' => 'top: auto; bottom: var(--thumbs-v-offset, 0px); --thumbs-translate-y:0; --justify-v-thumb-content: flex-end; --swiper-t-margin: auto; --swiper-b-margin: 0;',
				],
				'toggle'               => false,
				'selectors'            => [
					'{{WRAPPER}} .thumbs-swiper' => '{{VALUE}};',
		...
		$this->add_control(
			...
			[
				...
				'default'              => 'left',
				'selectors_dictionary' => [
					'left'   => 'left: var(--thumbs-h-offset); --thumbs-translate-x:0; --justify-h-thumb-content: flex-start;',
					'center' => 'left: calc(50% + var(--thumbs-h-offset, 0px)); --thumbs-translate-x:-50%;   --justify-h-thumb-content: center;',
					'right'  => 'left: calc(100% - var(--thumbs-h-offset, 0px)); --thumbs-translate-x:-100%;  --justify-h-thumb-content: flex-end;',
				],
				'selectors'            => [
					'{{WRAPPER}} .thumbs-swiper' => '{{VALUE}};',
				],
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'  => [
					'{{WRAPPER}} .thumbs-swiper' => '--thumbs-v-offset: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'  => [
					'{{WRAPPER}} .thumbs-swiper' => '--thumbs-h-offset: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_responsive_control(
			...
			[
				...
				'selectors'  => [
					'{{WRAPPER}} .thumbs-swiper' => '--max-width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_responsive_control(
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .thumbs-swiper' => '--thumb-item-size: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_responsive_control(
	...
	protected function add_thumbs_arrows_style_controls() {
		...
		$this->add_thumb_arrow_position_styles( 'prev_', esc_html__( 'Prev Arrow Position', 'the7mk2' ) );
		$this->add_thumb_arrow_position_styles( 'next_', esc_html__( 'Next Arrow Position', 'the7mk2' ) );
	...
	protected function add_thumb_arrow_style_states_controls( $prefix_name, $box_name ) {
		...
		$selector         = "{{WRAPPER}} .thumbs-swiper .the7-thumbs-swiper-button{$is_hover}";
	 ...
	 * @return void
	 */
	protected function add_thumb_arrow_position_styles( $prefix, $heading_name ) {
		$button_class  = '';
		$default_h_pos = 'left';
		if ( $prefix === 'next_' ) {
			$swiper_button_class = '.the7-thumbs-swiper-button-next';
			$default_h_pos       = 'right';
		} elseif ( $prefix === 'prev_' ) {
			$swiper_button_class = '.the7-thumbs-swiper-button-prev';
		}

		$selector = '{{WRAPPER}} .thumbs-swiper ' . $swiper_button_class;

		$this->add_control(
			$prefix . 'thumbs_arrow_position_heading',
			[
				'label'     => $heading_name,
				'type'      => Controls_Manager::HEADING,
				'separator' => 'before',
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'  => [
					$selector => '--thumbs-arrow-v-offset: {{SIZE}}{{UNIT}};',
				],
			]
		);

		$this->add_responsive_control(
			$prefix . 'thumbs_arrow_h_offset',
			[
				'label'      => esc_html__( 'Horizontal Offset', 'the7mk2' ),
				'type'       => Controls_Manager::SLIDER,
				'default'    => [
				...
				'selectors'  => [
					$selector => '--thumbs-arrow-h-offset: {{SIZE}}{{UNIT}};',
				],
			]
		);
	}
