...
function ultra_core(){
    ...
    /*----------- Check Permissions - End---------------*/
        ...
        if($ultra_css_ver != ""){
            ...
            add_action('init', 'ultra_email_settings');
            ...
            if($loginside)
            {
                ...
                add_action('login_enqueue_scripts', 'ultra_login_custom_css', 99);
            }
            ...
            if($get_menumng_page != "disable"){
                add_filter('admin_body_class', 'ultra_menumng_body_classes');
            }
...
function ultra_menumng_body_classes($classes) {
...
function ultra_login_custom_css(){
       ...
       $str = "";
        ...
        echo "<style type='text/css' id='ultra-custom-login-css'>".$str."</style>";
}
...
function ultra_footer_admin () {
    ...
    $get_menumng_page = ultra_get_option( "ultraadmin_menumng_page","enable");
    if($get_menumng_page != "disable"){
        ultra_menu_management_counts();
    }
}


function ultra_menu_management_counts(){
...
function ultra_email_settings(){
    global $ultraadmin;
    $ultraadmin = ultraadmin_network($ultraadmin);       

    if(isset($ultraadmin['from-mail-email']) && trim($ultraadmin['from-mail-email']) != ""){
       // wp_mail_from
        add_filter('wp_mail_from', 'ultra_from_mail');
    
    }     

    if(isset($ultraadmin['from-mail-name']) && trim($ultraadmin['from-mail-name']) != ""){
        // wp_mail_from_name
        add_filter('wp_mail_from_name', 'ultra_from_mail_name');
    }


}

function ultra_from_mail($original_email_address) {

    global $ultraadmin;
    $ultraadmin = ultraadmin_network($ultraadmin);       
    $ret = $ultraadmin['from-mail-email'];
    // $ret = "info@domain.com";
    return $ret;
}

function ultra_from_mail_name($original_email_address_name) {
     global $ultraadmin;
     $ultraadmin = ultraadmin_network($ultraadmin);       
     $ret = $ultraadmin['from-mail-name'];
    //  $ret = "info";
     return $ret;
 }
